...
def gen_srt_block(time_block, text):
...
def extract_marks(text_file):
    ...
    for line in lines:
        try:
            ...
            marks.append({
                'text':parts[0],
                'time_block_index':parts[1].split(',')
            })
        except:
            continue
    return marks
...
def gen_srt(id):
    time_blocks = t.open_pickle('%s.pkl' % id)
    marks = extract_marks('%s.txt' % id)
    f = open('%s.srt' % id,'w')
    for mark in marks:
        text = mark['text']
        ...
        a = gen_srt_block(time_block, text)
        f.write(a)
    f.close()
